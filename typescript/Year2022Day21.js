const SAMPLE_MONKEYS_AS_TEXT = `root: pppw + sjmn
dbpl: 5
cczh: sllz + lgvd
zczc: 2
ptdq: humn - dvpt
...
drzm: hmdt - zczc
hmdt: 32`;
const REAL_MONKEYS_AS_TEXT = `lsdv: fbnj * rhjt
jggb: rdpd + cqzw
ctqr: hnbn / zprm
qpmw: 5
vqhj: tbhz * jflr
...
nqwn: nzdt * jwbw
nwbd: 10`;
function parseIntoMonkeys(input) {
    const map = new Map();
    input.split(/\r?\n/)
        .map(parseIntoMonkey)
        .forEach((monkey) => {
...
function parseIntoMonkey(input) {
    const split = input.split(/: /);
    const first = split[0];
    const second = split[1];
    if (/^[0-9]+$/.test(second)) {
        return { name: first, rawValue: Number(second), left: null, operator: null, right: null };
    }
    else {
        const parts = second.split(/ /);
        return { name: first, rawValue: null, left: parts[0], operator: parts[1], right: parts[2] };
    }
}
function getValuePuzzle1(name, monkeys) {
...
function getValuePuzzle2(name, monkeys) {
    if (name == "humn") {
        return "x";
    }
    const monkey = monkeys.get(name);
