...
public class Day07 {
    ...
    private static long getCostPuzzle1(int[] positions, int position) {
        long cost = 0L;
    ...
    private static long getCostPuzzle2(int[] positions, int position) {
        long cost = 0L;
        for (int item : positions) {
            long difference = abs(item - position);
            cost += difference * (difference + 1) / 2; // See: https://www.cuemath.com/sum-of-integers-formula/
        }
        return cost;
    }

    private static long getBestFuel(int[] positions, int puzzle) {
        int minPosition = Arrays.stream(positions)
                                .min()
                                .getAsInt();
        int maxPosition = Arrays.stream(positions)
                                .max()
                                .getAsInt();

        long bestCost = Long.MAX_VALUE;
        for (int position = minPosition; position <= maxPosition; position++) {
            long cost = puzzle == 1 ? getCostPuzzle1(positions, position) : getCostPuzzle2(positions, position);
            if (cost < bestCost) {
                bestCost = cost;
            }
        }
    ...
    public static void testWithExamplesForPuzzle1() {
        ...
        System.out.println("Best fuel: " + getBestFuel(SAMPLE_POSITIONS, 1));
    }

    public static void testWithExamplesForPuzzle2() {
        System.out.println("### Day 07: Examples for puzzle 2 ###");
        System.out.println("Best fuel: " + getBestFuel(SAMPLE_POSITIONS, 2));
    }

    public static long doPuzzle1() {
        return getBestFuel(INPUT_POSITIONS, 1);
    }

    public static long doPuzzle2() {
        return getBestFuel(INPUT_POSITIONS, 2);
    }
}
